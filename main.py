...
offset = -2
mon_world = {"top": 286+offset, "left": 1073, "width": 373, "height": 148}
...
while True:
    ...
    for i in range(len(regions)):
        ...
        match_y = y_gene.find_max(current_gene, threshold=0.45)
        match_g = g_gene.find_max(current_gene, threshold=0.45)
        match_h = h_gene.find_max(current_gene, threshold=0.45)
        match_x = x_gene.find_max(current_gene, threshold=0.45)
        match_w = w_gene.find_max(current_gene, threshold=0.45)
        ...
        cv.imshow(f'test {i}', current_gene)
